...
<div class="col-12">
    ...
    <br>
                ...
                <table class="table table-head-fixed  col-12">
                  <thead>
                    <tr>
                      ...
                      <th class="col-4">Class Product</th>
                      <th class="col-4" colspan="2">Action</th>
                    </tr>
                  </thead>
                  @foreach ($prodclass as $prodclass)
                  <tbody>
                    <tr>
                      <td>{{$prodclass->id}}</td>
                      <td>{{$prodclass->product_class}}</td>
                      <td>
                           
                          <a href="#"> <button type="button" class="btn btn-warning" data-toggle="modal" data-target="#modal-edit">Update</button></a>
                          @include('layouts.modal.editclass') 
                      </td>
                      <td>
                        
                      <form action="{{route('delete-classproduct',$prodclass->id)}}" method="post">
                            @csrf
                            @method('DELETE')
          ...
          @include('layouts.modal.createclass') 
